...
const Scorewindow = (props) => {
    const { id, page, score, globaltime, questionsettime, questiontime, numberofquestions, display, style, level, statustest } = props.info;
    ...
    return (
        ...
        <div className="start-mode" style={{display:(props.display === "active")?'block':'none'}}></div>
                    <div  className="start-window" style={{display:(props.display === "active")?'flex':'none'}}>
                        ...
                        <div className="btn-start" onClick={() => props.launchtest()}>Try again</div>             
                    </div>
        </div>
    );
};
